...
import (
	...
	"github.com/andrewmysliuk/jobhound_core/internal/ports"
)

// Pipeline wires pipeline ports and runs one collect → filter → score → dedup → notify pass.
...
func (p *Pipeline) Run(ctx context.Context) error {
	if p.Collector == nil || p.Filter == nil || p.Scorer == nil || p.Dedup == nil || p.Notify == nil {
		return fmt.Errorf("app.Pipeline: nil dependency")
	}

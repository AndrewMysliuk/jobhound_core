...
func (h *HTTPHandler) getStageJobs(w http.ResponseWriter, r *http.Request) {
	...
	page, limit, bucket, ok := apputils.ParseJobListQuery(r.URL.Query())
	if !ok {
		apputils.WriteAPIError(w, http.StatusBadRequest, "invalid_query", "invalid page, limit, or bucket query")
		return
	}
	if stage == 1 && bucket != "" {
		apputils.WriteAPIError(w, http.StatusBadRequest, "invalid_query", "bucket is only allowed for stages 2 and 3")
		return
	}
	...
	if errors.Is(err, slots.ErrInvalidJobListQuery) {
		apputils.WriteAPIError(w, http.StatusBadRequest, "invalid_query", "invalid bucket query parameter")
		return
	}
	if err != nil {

...
- Pairs that remain **`PASSED_STAGE_2`** because they were **not** selected in that execution **stay** `PASSED_STAGE_2` (cap backlog) until a **later** pipeline-run execution or an explicit **“process next batch”**-style action (product draft §4; API/workflow shapes in **`011`** when implemented).
...
7. Per-job **manual** stage-3 rescoring is **out** of v1; **batch** continuation for cap backlog may be **`011`** — **policy** stays in this spec.

...
| 6   | [Built In](https://builtin.com/jobs/remote)    | **MVP** | **T2 (fact)**; **T3** for transport by default | `resources/builtin.md` — same listing/detail **URLs** as T2; **default** **Rod** (`internal/collectors/browserfetch`) when Chromium is available; **`JOBHOUND_BROWSER_ENABLED=0`** to force **`net/http`** only; **`JOBHOUND_COLLECTOR_BUILTIN_USE_BROWSER=0`** to keep the browser for other sources but HTTP for Built In; JSON-LD unchanged; **no fetch** when slot search empty; **delay** (env) |

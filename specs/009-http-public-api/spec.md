...
- **Filters / stage parameters**: update **stage-2** and/or **stage-3** (non–stage-1) inputs. Mutations must apply **§5** reset semantics (wipe dependent outcomes, including **manual marks** in the wiped scope) before or as part of the same logical operation; then client or server follows with the appropriate **`008`** run kind (2+3 vs 3-only).
- **Runs / actions**: start or query **user-initiated** pipeline work: stage-1 ingest (full first path vs **delta** later), stage 2+3 recompute, stage 3 only, **compound** “pull new then 2 then 3” if the product exposes one button, and **explicit “process next batch”** for LLM cap backlog (**`007`**). Request/response fields align with **`008`** contracts (workflow id, optional **`pipeline_run_id`**, coarse counts, errors)—**`009`** does not invent parallel payload semantics.
...
- **Primary** product features deferred by draft §8: **scheduled** auto-runs (no numbered epic yet)—API may later add endpoints that start the same workflows, but MVP vertical is **manual/API-triggered** first.
...
- **`003`** Temporal client from API process to start **`008`** workflows.  
- **`004` / `006` / `007`** semantics and activity boundaries (API does not re-spec).  
- **`008`** manual workflow contracts—**source of truth** for run kinds and response DTOs **`009`** mirrors.  
- Future **scheduled refresh** epic (if any), **`010`** (observability)—only when extending the API for schedules or ops-focused telemetry surfaces.
...
- [`../008-manual-search-workflow/spec.md`](../008-manual-search-workflow/spec.md) — workflow payloads; scheduled triggers remain backlog.

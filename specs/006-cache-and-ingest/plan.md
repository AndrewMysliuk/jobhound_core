...
| 2 Migrations | `ingest_watermarks`; optional `pipeline_runs.broad_filter_key_hash` (depends on `007` `pipeline_runs` existing) |
...
| D4 | Watermark | Postgres table **`ingest_watermarks`** (`contracts/ingest-watermark-and-filter-key.md`). |
| D5 | Broad filter key | Canonical JSON → **SHA-256** hex; store on **`pipeline_runs.broad_filter_key_hash`** when column added by **`006`** migration (after `007` creates `pipeline_runs`). |

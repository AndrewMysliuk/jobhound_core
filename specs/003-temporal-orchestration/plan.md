...
**MVP draft alignment**: the global product draft defines **slots**, **reset-without-refetch**, and **idempotent** stage-3 work under retries. This plan stays **foundation-only**; those behaviors are specified in **`004`**, **`006`**, **`007`**, and consumer epics **`008`–`009`**, not in `003`.
...
- Real product workflows, ingest, and API-triggered runs — `006`, `008`, `009` (payloads include **`slot_id`** / reserved **`user_id`**; activities idempotent where caps or outcomes are written — draft §4).
- GCP worker topology and prod addressing — env-only note sufficient for `003`.
- Correlation / advanced observability — `010`.
